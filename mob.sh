...
then
    ...
    echo "Usage:"
    echo "'$0 init [session-name]'  # initialize a new mob session (need to do this once per mob session)"
    ...
    echo "'$0 reset [session-name]' # reset the mob session (to clean the mob session and its branch)"
fi
